...
bool CaloSlaveSD::processHits(uint32_t unitID, double eDepEM, double eDepHad, double tSlice, int tkID, int secondaryTkId, uint16_t depth) {
  PCaloHit aCal = PCaloHit(unitID, eDepEM, eDepHad, tSlice, tkID, secondaryTkId, depth);
  LogDebug("HitBuildInfo") << " Sent Hit " << aCal << " to ROU " << name_;
  hits_.push_back(aCal);
  return true;
}
