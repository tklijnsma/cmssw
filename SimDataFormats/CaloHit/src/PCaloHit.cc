...
PCaloHit::PCaloHit(unsigned int id, float eEM, float eHad, float t, int i, int iSecondary, uint16_t d)
    : myTime(t), myItra(i), mySecondaryItra(iSecondary), detId(id), myDepth(d) {
  myEnergy = eEM + eHad;
  myEMFraction = (myEnergy <= 0. ? 1. : eEM / myEnergy);
}
